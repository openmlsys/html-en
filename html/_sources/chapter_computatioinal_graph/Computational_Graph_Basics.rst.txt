...
dependencies between operators. Figure :numref:\ ``ch04/ch04-simpleDAG``
shows a computational graph example of
:math:`\bf{Z}`\ =ReLU\ :math:`(\bf{X}\times\bf{Y})`.

.. _ch04/ch04-simpleDAG:

.. figure:: ../img/ch04/simple-graph.png

   Simple computationalgraph
...
gradient transfer status. Table :numref:`ch04/ch4-tensor` describes
the main attributes of a tensor.

.. _ch04/ch4-tensor:

.. table:: Tensor attributes
   ...
   +------------+---------------------------------------------------------+
   | dim        | Number of axes (or dimensions). The value is 0 for a    |
   |            | scalar and 1 for a vector.                              |
   +------------+---------------------------------------------------------+
   | dtype      | Data type, such as bool, uint8, int16, float32, and     |
   |            | float64.                                                |
   +------------+---------------------------------------------------------+
...
:numref:`ch04/ch04-tensor1`, a tensor is ragged if it has variable
...
.. figure:: ../img/ch04/tensor-class.png
...
As shown in Figure :numref:`ch04/ch04-dependence`, if the Matmul1
...
depend on each other with transitive relations.

.. _ch04/ch04-dependence:

.. figure:: ../img/ch04/dependence.png

   Computationaldependencies
...
2. **Indirect dependency**: For example, the Add node indirectly depends
...
3. **Mutual independence**: For example, the graph shows no input/output
   dependency between Matmul1 and Matmul2, meaning that the two nodes
   are independent of each other.

In the computational graph shown in Figure
:numref:`ch04/ch04-recurrent`, the Add node indirectly depends on the
...
.. _ch04/ch04-recurrent:

.. figure:: ../img/ch04/recurrent.png

   Circulardependency


In machine learning frameworks, the *unrolling* method is used to
represent loop iterations. Figure :numref:`ch04/ch04-recurrent-1`
...
operators are assigned new identifiers to avoid circular dependencies.

.. _ch04/ch04-recurrent-1:

.. figure:: ../img/ch04/unroll.png
...
for conditional control, ``tf.while_loop`` for loop control, and
``tf.case`` for branch control). These operators are composites of
...
computational graphs of Code ``ch04/code1``. When running a model
containing ``if`` conditions, the program needs to know which branch of
...
:math:`\bf{C}` does not participate in computation due to conditional
control. Similarly, in the backward computational graph, tensor
:math:`\bf{C}` is skipped in gradient computation.

.. _ch04/ch04-if:

.. figure:: ../img/ch04/if.png

   Computational graphs of conditionalcontrol
...
forward pass and gradient computation. Code ``ch04/code2`` shows a
control loop example. In its unrolled loop, :math:`\bf{X_i}` and
:math:`\bf{W_i}` are the lists of intermediate result tensors to be
maintained.
...
The forward and backward computational graphs of Code ``ch04/code2`` are
shown in Figure :numref:`ch04/ch04-while`. The gradient of the control
...
.. figure:: ../img/ch04/while.png

   Computational graphs of loopcontrol
...
:math:`\bf{X}` is fed into the neural network, the data is propagated
forward one layer at a time in the computational graph, and the
intermediate variables are calculated and stored until :math:`\bf{Y}` is
...
*x*. If :math:`y=g(x)` and :math:`z=f(y)=f(g(x))`, the derivative of *z*
with respect to *x* is

.. math:: \frac{\partial z}{\partial x}=\frac{\partial z}{\partial y}\frac{\partial y}{\partial x}.
...
:eq:`ch04/chainrule` of composite functions with respect to scalars
as follows: Assuming that :math:`\bf{X}` is an *m*-dimensional tensor,
:math:`\bf{Y}` is an *n*-dimensional tensor, :math:`\bf{z}` is a 1D
vector, :math:`\bf{Y}=g(\bf{X})`, and :math:`\bf{z}=f(\bf{Y})`, the
partial derivative of :math:`\bf{z}` with respect to each element of
:math:`\bf{X}` is

.. math:: \frac{\partial z}{\partial x_i}=\sum_j\frac{\partial z}{\partial y_j}\frac{ \partial y_j}{ \partial x_i}.

:eqlabel:``eq:ch04/chainrule-1``

The equivalent form of Equation :eq:`ch04/chainrule-1` is

.. math:: \nabla_{\bf{X}}\bf{z} = (\frac{\partial \bf{Y}}{\partial\bf{X}})^{\top}\nabla_{\bf{Y}}\bf{z},

:eqlabel:``eq:ch04/chainrule-2``

where, :math:`\nabla_{\bf{X}}z` represents the gradient matrix of
:math:`z` with respect to :math:`\bf{X}`.

Figure :numref:`ch04/ch04-chain` shows the application of the chain
...
twice to obtain the predicted value :math:`\bf{Y}`, and then performs
...
weight parameters to be updated include :math:`\bf{W}` and
:math:`\bf{W_1}`.

.. _ch04/ch04-chain:

.. figure:: ../img/ch04/chain.png
...
transfer the gradient information to :math:`\bf{W}` and :math:`\bf{W_1}`
using the chain rule method? And why do we need to calculate the
gradients of non-parameter data :math:`\bf{X}` and :math:`\bf{X_1}`? To
...
forward propagation in three steps: (1) :math:`\bf{X_1}=\bf{XW}`; (2)
:math:`\bf{Y}=\bf{X_1W_1}`; and (3)
:math:`Loss=\frac{1}{2} (\bf{Y}-Label)^2`.
...
:eq:`ch04/chainrule-3` and :eq:`ch04/chainrule-4` to calculate
the gradients of the loss function with respect to parameters
:math:`\bf{W}` and :math:`\bf{W_1}`:

.. math:: \frac{\partial {\rm Loss}}{\partial \bf{W_1}}=\frac{\partial \bf{Y}}{\partial \bf{W_1}}\frac{\partial {\rm Loss}}{\partial \bf{Y}}

:eqlabel:``eq:ch04/chainrule-3``

.. math:: \frac{\partial {\rm Loss}}{\partial \bf{W}}=\frac{\partial \bf{X_1}}{\partial \bf{W}}\frac{\partial {\rm Loss}}{\partial \bf{Y}}\frac{\partial \bf{Y}}{\partial \bf{X_1}}

:eqlabel:``eq:ch04/chainrule-4``

Both Equations :eq:`ch04/chainrule-3` and
:eq:`ch04/chainrule-4` solve
:math:`\frac{\partial {\rm Loss}}{\partial \bf{Y}}`, which corresponds
to grad :math:`\bf{Y}` in Figure :numref:`ch04/ch04-chain`.
:math:`\frac{\partial {\rm Loss}}{\partial \bf{Y}}\frac{\partial \bf{Y}}{\partial \bf{X_1}}`
in Equation :eq:`ch04/chainrule-4` corresponds to grad
:math:`\bf{X_1}` in Figure :numref:`ch04/ch04-chain`. To calculate the
gradient of model parameter :math:`\bf{W}`, the gradient of intermediate
result :math:`\bf{X_1}` is calculated. This also answers the second
...
Because :math:`\bf{X_1}=\bf{XW}`, :math:`\bf{Y}=\bf{X_1W_1}`, and
Loss=\ :math:`\frac{1}{2}`\ (:math:`\bf{Y}`-Label):math:`^2`, Equations
:eq:`ch04/chainrule-3` and :eq:`ch04/chainrule-4` are expanded
to :eq:`ch04/chainrule-5` and :eq:`ch04/chainrule-6` according
to Equations :eq:`ch04/chainrule-2`, respectively. Then, we can
...
.. math:: \frac{\partial {\rm Loss}}{\partial \bf{W_1}}=\frac{\partial \bf{Y}}{\partial \bf{W_1}}\frac{\partial {\rm Loss}}{\partial \bf{Y}}=\bf{X_1}^\top(\bf{Y}-{\rm Label})

:eqlabel:``eq:ch04/chainrule-5``

.. math:: \frac{\partial {\rm Loss}}{\partial \bf{W}}=\frac{\partial \bf{X_1}}{\partial \bf{W}}\frac{\partial {\rm Loss}}{\partial \bf{Y}}\frac{\partial \bf{Y}}{\partial \bf{X_1}}=\bf{X}^\top(\bf{Y}-{\rm Label})\bf{W_1}^\top

:eqlabel:``eq:ch04/chainrule-6``

Equation :eq:`ch04/chainrule-5` uses intermediate result
:math:`\bf{X_1}` in the forward computational graph when calculating the
gradient of :math:`\bf{W_1}`. In equation :eq:`ch04/chainrule-6`,
both input :math:`\bf{X}` and parameter :math:`\bf{W_1}` are used for
calculating the gradient of parameter :math:`\bf{W}`. This answers the
...
gradient of each parameter in the graph.

Based on Figure :numref:`ch04/ch04-chain` and Equations
:eq:`ch04/chainrule-3`, :eq:`ch04/chainrule-4`,
:eq:`ch04/chainrule-5` and :eq:`ch04/chainrule-6`, when the
