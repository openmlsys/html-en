...
.. figure:: ../img/ch04/static.png

   Generating and executing a staticgraph
...
of input data. In the static graph defined in Code ``ch04/code4``, we
need to create a placeholder for input :math:`\bf{X}` in line 1. Because
...
.. _ch04/ch04-static-gen:

.. figure:: ../img/ch04/static_gen.png

   Generating a staticgraph
...
in Figure :numref:`ch04/ch04-static-gen`, the Add and ReLU operators
...
that the Add and ReLU operators in Code ``ch04/code4`` have been fused
...
Figure :numref:`ch04/ch04-dynamic-gen` shows the process of generating
a dynamic graph.

.. _ch04/ch04-dynamic-gen:

.. figure:: ../img/ch04/eager-gen.png

   Generating a dynamicgraph
...
:math:`\bf{X}`, the machine learning framework starts to generate a
...
:numref:`ch04/ch04-dynamic-gen`, if the conditional returns true, only
the Matmul operator node with respect to tensor :math:`\bf{W1}` is added
...
As shown in Figure :numref:`ch04/ch04-dynamic-gen`, when the Matmul
operator with respect to tensor :math:`\bf{W1}` is called, the framework
runs the Matmul operator to calculate the product of inputs
:math:`\bf{X}` and :math:`\bf{W1}`, and then records the operator and
tensor :math:`\bf{X}` that will participate in backpropagation based on
the backward computation process
Grad\_\ :math:`\bf{W1}`\ =Grad\_\ :math:`\bf{Y}*\bf{X}`, thereby
...
and cons, as described in Table :numref:\ ``ch04/ch4-graph``.
...
.. table:: Static graph vs. dynamic graph
   ...
   +-----------------+--------------------------+------------------------+
   | Control flow    | Specialized syntax       | Frontend syntax        |
   | implementation  |                          |                        |
   +-----------------+--------------------------+------------------------+
   | Performance     | Better, supporting wide  | Poor, supporting       |
   |                 | optimization strategies  | limited graph          |
   |                 |                          | optimizations          |
   +-----------------+--------------------------+------------------------+
   | Memory          | Low                      | High                   |
   | footprint       |                          |                        |
...
If the static approach is used to implement Code ``ch04/code5``, the
machine learning framework creates a complete computational graph.
Because tensors :math:`\bf{Y_1}` and :math:`\bf{Y_2}` are computed
...
intermediate variable :math:`\bf{Y}`. Conversely, if the dynamic
...
:math:`\bf{Y}`. To further reduce memory footprint, the static approach
...
.. _conversion_between_and_combination_of_dynamic_and_static_graphs:
...
Table :numref:`ch04/ch4-eagertoscript` lists the APIs for dynamic
graph to static graph conversion provided by major frameworks.

.. _ch04/ch4-eagertoscript:

.. table:: Dynamic graph to static graph conversion support of major
   frameworks

   +---------------------------------------------+------------------------+
   | Framework                                   | Dynamic Graph to       |
   |                                             | Static Graph           |
   |                                             | Conversion             |
   +=============================================+========================+
   | TensorFlow                                  |                        |
   +---------------------------------------------+------------------------+
   | where AutoGraph automatically transforms a  |                        |
   | control flow to                             |                        |
   +---------------------------------------------+------------------------+
   ...
   | MindSpore                                   |                        |
   +---------------------------------------------+------------------------+
   | ``context.set_context(mode=context.GRAPH_MO |                        |
   | DE)``:                                      |                        |
   | static graph mode,                          |                        |
   +---------------------------------------------+------------------------+
   | ``@ms_function``: builds a static graph     |                        |
   | from source code.                           |                        |
   +---------------------------------------------+------------------------+
   | PyTorch                                     |                        |
   +---------------------------------------------+------------------------+
   | ``torch.jit.trace()``: builds a static      |                        |
   | graph by tracing operators.                 |                        |
   +---------------------------------------------+------------------------+
   | PaddlePaddle                                |                        |
   +---------------------------------------------+------------------------+
   | ``paddle.jit.TracedLayer.trace()``: builds  |                        |
...
The *source code transformation*–based method can overcome the drawbacks
involved in the tracing method and also consists of two phases, as shown
in Figure :numref:`ch04/ch04-ast`. The first involves lexical and
...
graph. When the ``Y=add_and_relu(Y,b)`` statement is executed, the
...
the advantages of dynamic and static graphs.
