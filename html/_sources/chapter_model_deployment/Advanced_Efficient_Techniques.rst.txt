...
:math:`cT`, where :math:`0<c\leq1`. The standard procedure using the
target model to generate :math:`\gamma + 1` tokens would require a total
time of :math:`\gamma T + T`. In contrast, with speculative decoding,
where :math:`\gamma + 1` tokens are produced (:math:`\gamma` by the
...
:math:`\gamma cT + T`. If all :math:`\gamma` draft tokens are accepted
by the target model and :math:`c` is small enough to make
:math:`cT << T`, speculative decoding has the potential to significantly
reduce latency during the decoding process.

To further explain, if we denote :math:`\alpha = E(\beta)` where
:math:`\beta` is the acceptance rate with a given prefix and
:math:`E(\beta)` is a natural measure of how well the draft model can
approximate the target model assuming :math:`\beta`\ s are i.i.d., the
expected number of tokens generated by the speculative process is
:math:`\frac{1-\alpha^{\gamma+1}}{1-\alpha}` [@leviathan2023fast].
According to the speculative decoding time for one superstep
:math:`\gamma cT + T`, the expected time for generating one token with
speculative decoding is
:math:`\frac{(c\gamma+1)(1-\alpha)}{1-\alpha^{\gamma+1}}T`. By choosing
a good :math:`\gamma` and a well-aligned efficient draft model meaning
big :math:`\alpha` and small :math:`c`, the result is desired.

Nevertheless, as the value of :math:`\gamma` continues to rise, it
...
the likelihood of acceptance typically diminishes when :math:`\gamma`
...
spent on generating :math:`\gamma` tokens with the draft model
represented as :math:`\gamma cT` effectively becomes a complete waste of
time when compared to generating a single token directly with the target
model; in addition, the draft model is consuming the GPU memory.

Therefore, finding the best :math:`\gamma` or having a well-designed
...
this issue effectively. For example:
...
[@dao2022flashattention]. They noted the absence of *IO-awareness* – the
...
.. math:: \textbf{A} = Softmax(\frac{\textbf{QK}^T}{\sqrt{d_k}})\textbf{V}

:eqlabel:``equ:std_attn``

As :math:`d_k` is a scalar, we can simplify it into three parts:
...
1. Load **K, Q** from HBM, compute **:math:`S` = :math:`QK^T`**, and
   write **S** to the HBM.

2. Read **S** from HBM, compute **P** = :math:`Softmax`\ S, and write
   **P** to HBM.
...
**Tiling**: Assuming a vector :math:`x\in \mathbb{R}^D`, the basic
...
Softmax values together with the respective :math:`s(x)` and
:math:`m(x)`. Ultimately, aggregate **O** blocks, the outcomes of the
block-wise Softmax values with the multiplication of corresponding **V**
...
size of these blocks. For **K, Q** and **V**
:math:`\in\mathbb{R}^{N \times d}`, the block size is set to
:math:`\lfloor \frac{M}{4d} \rfloor` where M is the SRAM size and the
output block size is set to be
:math:`min(\lfloor \frac{M}{4d} \rfloor, d)` [@dao2022flashattention].
Post-computation of each block, the resulting output block along with
the corresponding :math:`s(x)` and :math:`m(x)` are transferred back to
