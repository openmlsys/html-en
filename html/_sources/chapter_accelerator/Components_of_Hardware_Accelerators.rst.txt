...
As shown in Figure :numref:\ ``ch06/ch06-gv100``, a GV100 GPU contains
...
3. **Global memory**: Both GPUs and CPUs are capable of reading from and
...
.. _ch06/ch06-compute-unit:

.. figure:: ../img/ch06/compute_unit.png
...
3. **2D Matrix Unit**: computes the inner product of a matrix and a
   ...
   achieves the performance of matrix multiplication.
...
Units. As shown in Figure :numref:\ ``ch06/ch06-SM``, each SM has 64
...
.. figure:: ../img/ch06/SM.png

   Volta GV100 SM


A Tensor Core is capable of performing one :math:`4\times4` matrix
...
.. _ch06/ch06-tensorcore:

.. figure:: ../img/ch06/tensor_core.png

   Tensor Core’s :math:`4\times4` matrix multiply-accumulateoperation


:math:`\bf{A}`, :math:`\bf{B}`, :math:`\bf{C}`, and :math:`\bf{D}` are
:math:`4\times4` matrices. Input matrices :math:`\bf{A}` and
:math:`\bf{B}` are FP16 matrices, and accumulation matrices
:math:`\bf{C}` and :math:`\bf{D}` can be either FP16 or FP32 matrices.
...
.. _ch06/ch06-davinci_architecture:

.. figure:: ../img/ch06/davinci_architecture.png

   Da Vinciarchitecture
...
Figure :numref:\ ``ch06/ch06-davinci_architecture`` shows the
...
Specifically, it can multiply two :math:`16\times16` matrices in a
single instruction — equivalent to completing 4096
(=:math:`16\times16\times16`) multiply-accumulate operations within an
extremely short time — with precision comparable to FP16 operations.
