...
[1](#tab:ch06/ch06-categorize){reference-type="ref"
reference="tab:ch06/ch06-categorize"} offers an overview of the
...
Code [\[lst:torchscript\]](#lst:torchscript){reference-type="ref"
reference="lst:torchscript"} illustrates the use of the Scripting method
to print a TorchScript IR graph.

``` {#lst:torchscript caption="TorchScript function example" label="lst:torchscript"}
import torch
...
Code [\[lst:torchscriptir\]](#lst:torchscriptir){reference-type="ref"
reference="lst:torchscriptir"} shows the structure of this IR graph.

``` {#lst:torchscriptir caption="TorchScript IR example" label="lst:torchscriptir"}
...
Code [\[lst:ANF\]](#lst:ANF){reference-type="ref" reference="lst:ANF"}
via the ANF grammar.

``` {#lst:ANF caption="ANF grammar" label="lst:ANF"}
...
Code [\[lst:JaxCode\]](#lst:JaxCode){reference-type="ref"
reference="lst:JaxCode"} displays the Jaxpr corresponding to a function.

``` {#lst:JaxCode caption="Jaxpr" label="lst:JaxCode"}
...
Code [\[lst:JaxPr\]](#lst:JaxPr){reference-type="ref"
reference="lst:JaxPr"}.

``` {#lst:JaxPr caption="Jaxpr structure" label="lst:JaxPr"}
...
Code [\[lst:MindIR\]](#lst:MindIR){reference-type="ref"
reference="lst:MindIR"}.

``` {#lst:MindIR caption="ANF expression" label="lst:MindIR"}
lambda (x, y)
    let a = x - 1 in
    let b = a + y in
    let func = lambda (x, y)
