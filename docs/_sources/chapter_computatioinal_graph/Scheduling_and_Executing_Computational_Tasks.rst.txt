...
In Figure :numref:\ ``ch04/ch04-diaoduzhixing``, the directed acyclic
...
in turn, and the scheduling ends.

.. _ch04/ch04-diaoduzhixing:

.. figure:: ../img/ch04/schedule.png

   Operator scheduling andexecution
...
.. _ch04/ch04-single-op-exec:

.. figure:: ../img/ch05/single_op_exec.PNG

   Operator schedulingtrace
...
2. **Parallel**: executes its tasks concurrently for higher
   ...
   thereby consuming more system resources.
...
must be guaranteed. Figure :numref:`ch04/ch04-diaodu` shows a
...
order to ensure that each operator receives the necessary input.

.. _ch04/ch04-diaodu:

.. figure:: ../img/ch04/order.png

   Serial operatorscheduling


A computational graph may also contain operators independent of each
other, for example, op1 and op2 shown in Figure
:numref:`ch04/ch04-para`. We can have each operator run on different
...
more computing resources at the same time.

.. _ch04/ch04-para:

.. figure:: ../img/ch04/para.png

   Parallel operator scheduling


Serial execution and parallel execution have their own advantages and
disadvantages, as summarized in Table :numref:`ch04/ch4-graph`.

.. _ch04/ch4-graph:

.. table:: Comparison between serial execution and parallel execution
...
-  **Python operators**: They run on the host CPU. Unlike CPU operators,
...
shown in Figure :numref:`ch04/ch04-tongbu`, in each iteration, a batch
...
data loading is idle at model training time.

.. _ch04/ch04-tongbu:

.. figure:: ../img/ch04/sync.png

   Synchronous mechanism


In the asynchronous setting shown in Figure :numref:`ch04/ch04-yibu`,
...
computing, as shown in Figure :numref:`ch04/ch04-yibubingxing`. On the
...
.. _ch04/ch04-yibubingxing:

.. figure:: ../img/ch04/para-async.png

   Asynchronous mechanism combined with parallelcomputing
