...
block movement, and other relevant factors.
...
shown in Figure :numref:`ch05/ch05-memory_architecture`. As we move up
the pyramid, the storage elements have a higher cost but a faster access
time.

.. _ch05/ch05-memory_architecture:

.. figure:: figs/ch05/memory_architecture.png

   Modern computer memoryhierarchy
...
(2) Spatial locality: the tendency to access nearby memory locations in
...
.. _ch05/ch05-parallel_computing:

.. figure:: figs/ch05/parallel_computing.png

   Serial computing and parallelcomputing
...
(3) How long does it take to access each value between those calculated
...
In Code `[lst:before_tvm] <#lst:before_tvm>`__, the basic computational
logic is as follows: Initialize tensor C, multiply tensor A by tensor B,
and accumulate the results to tensor C.
...
size 1024 :math:`\times` 1024, then the total memory required by the
tensors is 1024 :math:`\times` 1024 :math:`\times` 3 :math:`\times`
...
the tiny block formed by ``m.inner``\ :math:`\times`\ ``n.inner`` needs
...
:math:`\times` 32 :math:`\times` 3 :math:`\times` sizeof(float), which
is 12 KB in the cache. The optimized code is shown in Code
`[lst:after_tvm] <#lst:after_tvm>`__. We perform tiling on loops m and n
based on factor 32 as the previous analysis. Similarly, we tile the loop
k based on factor 4, then reorder the k.outer and k.inner axis as the
outermost axis.
...
Code `[lst:before_poly] <#lst:before_poly>`__ is used as an example to
...
As shown in Figure :numref:`ch05/ch05-poly_test`, a memory access
...
.. _ch05/ch05-poly_test:

.. figure:: figs/ch05/poly_test.png

   Polyhedral model of the samplecode
...
shown in Code `[lst:after_poly] <#lst:after_poly>`__.
...
.. figure:: figs/ch05/poly.png

   Optimization result with the polyhedralmodel
...
threads (SIMT). As shown in Figures :numref:`ch05/ch05-SIMD` and
:numref:`ch05/ch05-SIMT`, respectively, SIMD corresponds to chips with
