...
In this equation, for a sufficiently small value of the step size
:math:`h`, the difference quotient :math:`\frac{f(x+h)-f(x)}{h}` is used
...
theoretically diminishes as the value of :math:`h` approaches zero. This
...
systems. As the value of :math:`h` decreases, the round-off error
conversely increases, adding a degree of uncertainty to the computation.

This creates a complex interplay between truncation error and round-off
error. When the value of :math:`h` is large, the truncation error
dominates, whereas when :math:`h` is small, the round-off error is more
significant. Consequently, the total error of numerical differentiation
achieves a minimum at an optimal :math:`h` value that balances these two
types of errors.
...
.. math:: \frac{\partial}{\partial x}(f(x)+g(x))\rightsquigarrow\frac{\partial}{\partial x}f(x)+\frac{\partial }{\partial x}g(x)

.. math:: \frac{\partial}{\partial x}(f(x)g(x))\rightsquigarrow(\frac{\partial}{\partial x}f(x))g(x)+f(x)(\frac{\partial}{\partial x}g(x))
...
Consider a composite function :math:`y=a(b(c(x)))`. The formula to
calculate its gradient, :math:`\frac{\partial y}{\partial x}`, is given
as:

.. math:: \frac{\partial y}{\partial x}=\frac{\partial y}{\partial a}\frac{\partial a}{\partial b}\frac{\partial b}{\partial c}\frac{\partial c}{\partial x}
...
:math:`\frac{\partial y}{\partial x_1}` at the point
:math:`(x_1, x_2)=(2,5)`:

.. math:: y=f(x_1,x_2)=ln(x_1)+{x_1}{x_2}-sin(x_2)
...
.. _ch04/ch04-calculation_graph:

.. figure:: ../img/ch04/AD-example_graph.png

   Computational graph of the examplefunction


Forward Mode
~~~~~~~~~~~~

.. _ch04/ch04-forward-mode-compute-function:

.. figure:: ../img/ch04/AD-forward_example.png

   Illustration of forward-mode automaticdifferentiation
...
:math:`{\dot{v}_i}=\frac{\partial v_i}{\partial x_1}` from top to
bottom, as depicted on the right. Consequently, this leads to the
computation ofthe final variable
:math:`{\dot{v}_5}=\frac{\partial y}{\partial x_1}`. In the process of
...
a function :math:`f:{\mathbf{R}^n}\to \mathbf{R}^m`, where :math:`n` is
the number of independent input variables :math:`x_i`, and :math:`m` is
thenumber of independent output variables :math:`y_i`, the derivative
resultscorrespond to the following Jacobian matrix:
...
Each forward pass of function :math:`f` results in partial derivatives
...
Therefore,executing :math:`n` forward passes gives us the full Jacobian
matrix.
...
The forward mode allows us to compute Jacobian-vector products
byinitializing :math:`\dot{\mathbf{x}}=\mathbf{r}` to generate the
...
toevaluate the derivatives of :math:`f` propagated from inputs to
...
Figure :numref:\ ``ch04/ch04-backward-mode-compute`` illustrates
...
:math:`\bar{v}_5=\bar{y}=\frac{\partial y}{\partial y}=1`, we
sequentiallycompute each intermediate variable
:math:`{\bar{v}_i}=\frac{\partial y_j}{\partial v_i}` from bottom to
...
variables :math:`{\bar{x}_1}=\frac{\partial y}{\partial x_1}` and
:math:`{\bar{x}_2}=\frac{\partial y}{\partial x_2}`.

.. _ch04/ch04-backward-mode-compute:

.. figure:: ../img/ch04/AD-backward_example.png

   Illustration of reverse-mode automaticdifferentiation


Every reverse pass of function :math:`f` produces partial derivatives of
...
matrix.Consequently, executing :math:`m` reverse passes gives us the
full Jacobianmatrix.

.. math::


   \begin{bmatrix}        \frac{\partial y_j}{\partial x_1} & \cdots & \frac{\partial y_j}{\partial x_n}    \end{bmatrix}

\ Similarly, we can compute vector-Jacobian products to obtain the
resultsfor a single row.\ 

.. math::

   \mathbf{r}^{T}\mathbf{J}_{f}=    \begin{bmatrix}        r_1 & \cdots & r_m    \end{bmatrix}    \begin{bmatrix}        \frac{\partial y_1}{\partial x_1} & \cdots & \frac{\partial y_1}{\partial x_n} \\        
   \vdots & \ddots & \vdots \\        
   \frac{\partial y_m}{\partial x_1} & \cdots & \frac{\partial y_m}{\partial x_n}    \end{bmatrix}
...
for a given function :math:`f`. This characteristic is particularly
...
:math:`(f:{\mathbf{R}^n}\to \mathbf{R}^m, n << m)`, the forward mode
proves to be more efficient. Conversely, when the function has
considerably more inputs than outputs
:math:`(f:{\mathbf{R}^n}\to \mathbf{R}^m, n >> m)`, the reverse mode
becomes advantageous.

For an extreme case where the function maps from :math:`n` inputs to a
single output :math:`f:{\mathbf{R}^n}\to \mathbf{R}`, we can evaluate
all the derivatives of the output with respect to the inputs
:math:`(\frac{\partial y}{\partial x_1},\cdots,\frac{\partial y}{\partial n})`
using a single reverse pass or :math:`n` forward passes. This is a
situation akin to derivative evaluation for a multi-input, single-output
network, a structure frequently encountered in machine learning.
...
:math:`a=(x+y)/z` as an example; it needs to be manually decomposed as
follows:
...
in Code \ ``lst:OO``, we utilize the AutoDiff code from automatic
