...
.. _ch-deploy/model_obfuscate:

.. figure:: ../img/ch08/model_obfuscate.png

   Procedure of modelobfuscation


Figure :numref:`ch-deploy/model_obfuscate` depicts the model
obfuscation procedure, which is described as follows.
...
2.  **Scramble the network structure of the computational graph [1]_:**
...
4.  **Scramble weights of the computational graph:** Add random noise
    ...
    result. The computational graph processed after Steps (2), (3), and
    (4) are then saved as a model file for subsequent operations.
...
.. [1]
   Scrambling refers to adding noise to the computational graph. Common
   methods include adding redundant nodes and edges and merging some
   subgraphs.
