...
Figure :numref:`ch010/ch10-datacentre` shows the typical architecture
of a machine learning cluster. There are many servers deployed in such a
...
:numref:`ch010/ch10-datacentre`, when server 1 and server 2 send data
to server 3 through their respective network links (say 10 Gb/s of
...
.. _ch010/ch10-datacentre:

.. figure:: ../img/ch10/ch10-datacentre.png

   Architecture of a machine learningcluster
...
hundreds of billions of parameters (e.g., GPT-3 — a huge language model
released by OpenAI — has nearly 175 billion parameters). If each
...
:math:`*` 4 bytes) of local gradient data in each round of training
iteration. If there are three model replicas, at least 1.4 TB [700 GB
:math:`*` :math:`(3-1)`] of gradient data needs to be transmitted. This
is because for :math:`N` replicas, only :math:`N-1` of them need to be
transmitted for computation. To ensure that the model replicas will not
diverge from the parameters in the main model, the average gradient —
once computed — is broadcast to all model replicas (1.4 TB of data) for
updating local parameters in these model replicas.
...
Ethernet links ranges from 10 Gb/s to 25 Gb/s.  [1]_ Using Ethernet to
...
server 1 in Figure :numref:`ch010/ch10-datacentre` as an example. This
server is equipped with two CPUs, which communicate with each other
through QuickPath Interconnect (QPI). Within a CPU interface (socket),
the accelerator and CPU are connected by a PCIe bus. Accelerators use
...
.. [1]
   Network bandwidth is typically measured in Gb/s, whereas memory
   bandwidth is in GB/s — *b* stands for bit, and *B* stands for byte.
