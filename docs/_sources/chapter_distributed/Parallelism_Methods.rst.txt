...
.. _ch010/ch10-single-node:

.. _ch010/ch10-parallel-methods:

.. figure:: ../img/ch10/ch10-single-node.png

   Single-node trainingsystem
...
``ch010/ch10-parallel-methods`` compiles various partition methods.
...
single data" or *model parallelism*. When training exceptionally large
...
*hybrid parallelism* paradigm.

:Parallelism methods \| Classification \| Single Data \| Multiple Data
\| \|——————|———————–|——————– \| \| Single program \| single-node
...
training batch size is :math:`N`, and that there are :math:`M` devices
available for parallel acceleration. To achieve data parallelism, the
batch size is partitioned into :math:`M` partitions, with each device
getting :math:`N/M` training samples. Sharing a replica of the training
program, each device executes and calculates a gradient separately over
its own data partition. Each device (indexed :math:`i`) calculates a
gradient :math:`G_i` based on local training samples. To ensure that
training program parameters are coherent, local gradients :math:`G_i` on
different devices are aggregated to calculate an average gradient
:math:`(\sum_{i=1}^{N} G_i) / N`. To complete the training on this
mini-batch, the training program updates model parameters based on the
average gradient.

Figure :numref:`ch010/ch10-data-parallel` shows a data-parallel
...
communication operation) to calculate an average gradient.

.. _ch010/ch10-data-parallel:

.. figure:: ../img/ch10/ch10-data-parallel.png
...
parallel execution. Assume that the operator has :math:`P` parameters
and the system consists of :math:`N` devices. To minimize the workload
on each device given the limited memory capacity, we can evenly assign
the parameters across the devices (:math:`P/N` = number of parameters
per device). This partitioning method is called **intra-operator
parallelism**, which is a typical application of model parallelism.

Figure :numref:`ch010/ch10-model-parallel-intra-op` shows an example
...
.. _ch010/ch10-model-parallel-intra-op:

.. figure:: ../img/ch10/ch10-model-parallel-intra-op.png

   Model-parallel system: intra-operatorparallelism


In some cases, the overall model — rather than specific operators —
requires more memory than a single device can provide. Given :math:`N`
operators and :math:`M` devices, we can evenly assign the operators
across :math:`M` devices. As such, each device needs to run forward and
backward computation of only :math:`N/M` operators, thereby reducing the
memory overhead of each device. This application of model parallelism is
called *inter-operator parallelism*.

Figure :numref:`ch010/ch10-model-parallel-inter-op` shows an example
...
.. _ch010/ch10-model-parallel-inter-op:

.. figure:: ../img/ch10/ch10-model-parallel-inter-op.png

   Model-parallel system: inter-operatorparallelism
...
hybrid parallelism. Figure :numref:`ch010/ch10-hybrid-parallel` shows
...
.. _ch010/ch10-hybrid-parallel:

.. figure:: ../img/ch10/ch10-hybrid-parallel.png
