...
into multiple micro-batches. For example, if a mini-batch with :math:`D`
training samples is divided into :math:`M` micro-batches, then each
micro-batch has :math:`D/M` data samples. These micro-batches enter the
...
Figure :numref:`ch010/ch10-pipeline-parallel` shows an execution
...
that :math:`F_{i,j}` represents the :math:`i`\ th forward computation
task of the :math:`j`\ th micro-batch, and :math:`B_{i,j}` represents
the :math:`i`\ th backward computation task of the :math:`j`\ th
micro-batch. After completing the forward computation of the first
micro-batch (denoted as :math:`F_{0,0}`), device 1 sends the
intermediate result to device 2, which then starts the corresponding
forward computation task (denoted as :math:`F_{1,0}`). At the same time,
device 1 starts the forward computation task of the second micro-batch
(denoted as :math:`F_{0,1}`). Forward computation will be finally
completed on the last device in the pipeline, that is, device 3.

.. _ch010/ch10-pipeline-parallel:

.. figure:: ../img/ch10/ch10-pipeline-parallel.png

   Pipeline-parallelsystem
...
:math:`B_{3,0}`). On completion, the intermediate result is sent to
device 3, which then starts the corresponding backward computation task
(denoted as :math:`B_{2,0}`). At the same time, device 4 caches the
gradient of the first micro-batch and starts computation for the second
micro-batch (denoted as :math:`B_{3,1}`). When device 4 completes all
...
gradient. This gradient is used to update model parameters.
...
shown in Figure :numref:`ch010/ch10-pipeline-parallel`, after
...
tasks. This waiting time is referred to as *pipeline bubbles*. To
...
computing cores in an accelerator).
