...
mapping conversion on the raw features: *Male* :math:`\rightarrow` 0,
*Female* :math:`\rightarrow` 1, and *Unknown* :math:`\rightarrow` 2. The
...
.. figure:: ../img/ch_recommender/feature_store.png

   Example of a featurestore
...
model, though there are subtle distinctions.

.. _recommendation models:

.. figure:: ../img/ch_recommender/recommendation_model.png

   Structure of a recommendation model
...
:numref:`recommendation models`. This discrepancy can be attributed to
the plethora of discrete features in recommendation systems, each
necessitating an individual embedding item for every possible value.
...
corresponds to a user) of :math:`4*64*10^8~=23.8` GB in size.
...
:math:`\frac{1}{100000}` of the total embedding items are accessed
concurrently.
...
.. _parameter server in recommendation:

.. figure:: ../img/ch_recommender/parameter_server_in_recommendation.png

   Role of parameter servers in a recommender system
