...
ach element in matrix :math:`C` is independently computed, and numerous
GPU threads can be launched to compute the corresponding elements in
matrix :math:`C` in parallel. The GPU kernel function is shown in
Code \ ``lst:gpu``.
...
implementation. Each element in matrix :math:`C` is computed by one
thread. The row index :math:`m` and column index :math:`n` of the
element in matrix :math:`C` corresponding to the thread are computed in
lines 5 and 6 of the GPU kernel. Then, in lines 9 to 11, the thread
loads the row vector in matrix :math:`A` according to the row index and
the column vector in matrix :math:`B` according to the column index,
computes the vector inner product. The thread also stores the result
back to :math:`C` matrix in line 17.

.. _cuda_naive_gemm:

.. figure:: ../img/ch06/practise/naive.png

   Simple implementation ofGEMM
...
Each thread block processes :math:`16\times16` elements in matrix
:math:`C`. Therefore, :math:`(M - 1) / 16 + 1 \times (N - 1) / 16 + 1`
thread blocks are used to compute the entire matrix :math:`C`.
...
`first_attempt.cu <https://github.com/openmlsys/openmlsys-cuda/blob/main/first_attempt.cu>`__.
...
formula: 2 :math:`\times` Frequency :math:`\times` Number of
...
single-precision compute units in each SM. The results are as follows:
...
multiplication :math:`A\times B`. Its time complexity is
:math:`O(M*N*K)`, whereas that time complexity of the entire computing
process is :math:`O(M*N*K+2*M*N)`. Therefore, optimizing matrix
multiplication is key to improving performance.
...
product within :math:`K` loops, floating-point multiplication and
addition operations occur each time elements from matrix :math:`A` and
:math:`B` are loaded. Consequently, the arithmetic intensity is 1,
derived from two 32-bit floating-point operations divided by two 32-bit
data load/store instructions.

In the original code, each thread handles one element in matrix
:math:`C`, computing the inner product of a row in matrix :math:`A` and
a column in matrix :math:`B`. In essence, we can elevate the arithmetic
intensity by amplifying the elements in matrix :math:`C` that each
thread can process, computing the inner product of multiple rows in
matrix :math:`A` and multiple columns in matrix :math:`B`. More
specifically, if :math:`m` elements in matrix :math:`A` and :math:`n`
elements in matrix :math:`B` are loaded concurrently while calculating
the inner product in :math:`K` loops, there are :math:`m+n` 32-bit
load/store instructions and :math:`2mn` 32-bit computational
instructions. Hence, the arithmetic intensity becomes
:math:`\frac{2mn}{m+n}`. Therefore, by increasing :math:`m` and
:math:`n`, we can optimize the arithmetic intensity.
...
`util.cuh <https://github.com/openmlsys/openmlsys-cuda/blob/main/util.cuh>`__.

Note that each thread needs to load four ``float`` numbers (instead of
one) from matrix :math:`A` and matrix :math:`B`, requiring each thread
to process :math:`4\times 4` blocks (``thread tile``) in matrix
:math:`C`. Each thread loads data from matrix :math:`A` and matrix
:math:`B` from left to right and from top to bottom, computes the data,
and stores the data to matrix :math:`C`, as shown in
Figure :numref:\ ``use_float4``.

.. _use_float4:

.. figure:: ../img/ch06/practise/use_float4.png

   Enhancing arithmeticintensity


For details about the complete code, see
`gemm_use_128.cu <https://github.com/openmlsys/openmlsys-cuda/blob/main/gemm_use_128.cu>`__.
...
achieve this, see
`gemm_use_tile.cu <https://github.com/openmlsys/openmlsys-cuda/blob/main/gemm_use_tile.cu>`__.

.. _use_tile:

.. figure:: ../img/ch06/practise/use_tile.png

   Further enhancement of the arithmetic intensity by adding
   matrixblocks processed by eachthread
...
which processes one or more matrix blocks in matrix :math:`C`. As shown
in Figure :numref:\ ``duplicated_data``, thread :math:`x` and thread
:math:`y` process the same row in matrix :math:`C`, so they load the
same data from matrix :math:`A`. The shared memory can be used to
improve the program throughput by enabling different threads in the same
thread block to load unique data and reuse shared data.

.. _duplicated_data:

.. figure:: ../img/ch06/practise/duplicated_data.png

   Threads loading redundantdata


We have previously mentioned that the inner product can be computed by
loading and accumulating data in :math:`K` loops. Specifically, in each
loop, threads that process the same row in matrix :math:`C` load the
same data from matrix :math:`A`, and threads that process the same
column in matrix :math:`C` load the same data from matrix :math:`B`.
However, the code needs to be optimized by dividing :math:`K` loops into
:math:`\frac{K}{tileK}` outer loops and :math:`tileK` inner loops. In
this way, an entire block of data is loaded in each outer loop and
accumulated in each inner loop. Figure :numref:\ ``use_smem_store``
shows the process of moving data from the global memory to the shared
memory. Before each inner loop starts, the entire ``tiles`` in matrix
:math:`A` and matrix :math:`B` is stored in the shared memory.

Figure :numref:\ ``use_smem_load`` shows the process of moving data from
...
the data it requires from the shared memory.

.. _use_smem_store:

.. figure:: ../img/ch06/practise/use_smem_store.png

   Writing data to the sharedmemory


.. _use_smem_load:

.. figure:: ../img/ch06/practise/use_smem_load.png

   Loading data from the sharedmemory


For details about the complete code, see
`gemm_use_smem.cu <https://github.com/openmlsys/openmlsys-cuda/blob/main/gemm_use_smem.cu>`__.
...
``Pipe Fma Cycles Active`` also improves significantly, demonstrating
the benefits of the shared memory.

Reducing Register Usage
-----------------------

In previous sections, the data blocks that store matrix :math:`A` in the
...
For details about the complete code, see
`gemm_transpose_smem.cu <https://github.com/openmlsys/openmlsys-cuda/blob/main/gemm_transpose_smem.cu>`__.
...
time this instruction is issued during :math:`tileK` inner loops, the
...
Figure :numref:\ ``use_smem_pipeline``. Accessing the shared memory may
...
by optimizing the pipeline. Specifically, during :math:`tileK` inner
loops, loading instructions that prepare data in the next loop can be
loaded at the beginning of each loop, as shown in
Figure :numref:\ ``hide_smem_latency``. In this way, computation
...
not be blocked by the instructions that load the data for the next loop.

.. _use_smem_pipeline:

.. figure:: ../img/ch06/practise/use_smem_pipeline.png

   Pipeline of the previous GPU kernelfunction


.. _hide_smem_latency:

.. figure:: ../img/ch06/practise/hide_smem_latency.png

   Pipeline that hides the shared memory loadinglatency


For details about the complete code, see
`gemm_hide_smem_latency.cu <https://github.com/openmlsys/openmlsys-cuda/blob/main/gemm_hide_smem_latency.cu>`__.
...
``Stall Short Scoreboard`` decreases by 67% when compared with that of
...
decrease of ``Stall Short Scoreboard`` demonstrates that hiding the
...
section. At the beginning of each of the :math:`\frac{K}{tileK}` outer
loops, instructions that load the data tiles in matrix :math:`A` for the
...
after :math:`tileK - 1` loops are executed, further reducing the latency
of writing data to ``tile``. Figure :numref:\ ``hide_global_latency``
shows the optimized pipeline.

.. _hide_global_latency:

.. figure:: ../img/ch06/practise/hide_global_latency.png

   Pipeline that hides the global memory loadinglatency


For details about the complete code, see
`gemm_final.cu <https://github.com/openmlsys/openmlsys-cuda/blob/main/gemm_final.cu>`__.
...
-  Increasing parallelism through resource mapping: Multi-level parallel
   resources (``blocks``, ``warps``, and ``threads``) are mapped to the
   data needing computation and transfer to enhance program parallelism.

-  Reducing memory access latency through memory structure optimization:
   Based on the recognition of data reuse within the same ``block``
   during computation, the reused data is stored in local memory (like
   shared memory and registers) to increase locality.
